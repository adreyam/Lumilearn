...
function render() {
    ...
    renderNumbers();
    expr_div.textContent = expr.length === 0 ? "محاولتك" : expr.join(" ");
...
function onOperatorClick(op) {
    if (expr.length === 0) { setMessage("اختر رقماً أولاً", false); return; }
    if (["+", "-", "*", "/"].includes(expr[expr.length - 1])) { setMessage("لا يمكن وضع عمليتين متتاليتين", false); return; }
    snapshot();
    expr.push(op);
    render();
}
...
function attempt() {
    if (expr.length < 3) { setMessage("قم ببناء تعبير صحيح", false); return; }
    if (["+", "-", "*", "/"].includes(expr[expr.length - 1])) { setMessage("لا يمكن أن ينتهي التعبير بعملية", false); return; }
    let res;
    try { res = eval(expr.join(" ")); } catch { setMessage("تعبير غير صالح", false); return; }
    ...
    if (numbers.some(n => n === target)) {
        ...
        setMessage(`لقد فزت! +${b} نقطة`);
        setTimeout(generateGame, 2000);
    }
}

function resetScore() { saveScore(0); generateGame(); setMessage("تم إعادة ضبط النتيجة!"); }
function undo() {
    if (undoStack.length === 0) { setMessage("لا يوجد شيء للتراجع عنه", false); return; }
    let snap = undoStack.pop();
    numbers = snap.numbers; expr = snap.expr; used = snap.used; history = snap.history;
    render(); setMessage("تم التراجع بنجاح");
}
function restart() { numbers = [...initialNumbers]; expr = []; used = []; history = []; render(); setMessage("تمت إعادة التشغيل بنجاح"); }
